...
class poisson_process:
    ...
    def pop_model(self,f1):
        ...
        # modified：TypeError: only size-1 arrays can be converted to Python scalars

 # 检查 selector 的长度
        if len(selector) == 0:
            # 如果没有找到匹配的索引，可根据需求处理，这里简单返回 None
            return None
        elif len(selector) == 1:
            # 如果只有一个匹配的索引，正常提取模型
    ...
    def predict_poission_fit_mod(self,f1,f2,val='Poisson_lam',return_raw = False):
        if return_raw:
            ...
            # 检查 model 是否为 None
            if model is None:
                print("Warning: No model found for the given f1 when return_raw is True. Returning 0 as default.")
                return 0
            model = model[0]
        ...
        if self._variablity_lambda and val =='Poisson_lam':
            ...
            if self._todays_random is None:
                ...
                # 检查 model 是否为 None
                if model is None:
                    print("Warning: No model found for the given f1 when _todays_random is None. Returning 0 as default.")
                    return 0
            ...
            # 检查 model 是否为 None
            if model is None:
                print("Warning: No model found for the given f1. Returning 0 as default.")
                return 0
            ...
            try:
                ...
                # 检查 model 是否为 None
                if model is None:
                    print("Warning: No model found for the given f1 in except block. Returning 0 as default.")
                    return 0
                model = model[0]
                Model_t = model[(model[self.x_names[1]] == f2)]
                lam = Model_t['Poisson_lam']
            ...
            # 检查 model 是否为 None
            if model is None:
                print("Warning: No model found for the given f1 in else block. Returning 0 as default.")
                return 0
            model = model[0]
            Model_t = model[(model[self.x_names[1]] == f2)]
            lam = Model_t[val]
